...
This script holds some functions to download and clean DAHITI Virtual Stations data records 
for any study area using study area shapefile or geojson file.
As a output of these functions, two different folders are created, DAHITI_Raw and DAHITI_Processed, 
for ideal management of raw and processed metadata files. 


Notes
-----
A part of this function is adapted from https://dahiti.dgfi.tum.de/en/ api requesting example enlisted in their website.
...
def download_DAHITI_stations(path, roi_path):
    ...
    """This function downloades all the available virtual stations which are intersecting with the Region of Interest (ROI) shapefile or geojson file. It also 
    develops a basic metadata dataframe for the downloaded virtual stations.
    
    As output of the function, two folders are created. (i) DAHITI_Raw (ii) DAHITI_Processed  
    (i) DAHITI_Raw folder consists with csv files for each intersected virtual stations.
    (ii) DAHITI_Processed folder consists with a csv file of simple Metadata information of the virtual stations.
    ...
    roi_path : str
            Takes the file path of the region of interest. Only geojson or shapefiles are supported.
    ...
    if len(df) == 0:
        raise ValueError("There is no intersected DAHITI virtual stations for this ROI. Please try new ROI file")
